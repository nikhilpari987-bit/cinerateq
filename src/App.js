      ...
      <div className="modal-box" onClick={e => e.stopPropagation()}>
        ...
        <div ref={cardRef} className="share-card">
          <div className="share-card-top">
            <span className="share-logo">CINERATE</span>
            <span className="share-tag">Movie Review</span>
          </div>
          {movie.posterUrl && <img src={movie.posterUrl} alt={movie.title} className="share-poster" crossOrigin="anonymous" />}
          <div className="share-card-body">
            <div className="share-year">{movie.year} · {movie.genre}</div>
            <h3 className="share-title">{movie.title}</h3>
            <div className="share-stars">{"★".repeat(userRating)}{"☆".repeat(5-userRating)}</div>
            <div className="share-reviewer">— {userName}</div>
          </div>
        </div>
